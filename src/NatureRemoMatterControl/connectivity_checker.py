...
def check_router(router_ip: Optional[str], port: int = 80, timeout: float = 2.0):
    """ルーターへの到達性を確認します。"""
    if not router_ip:
        raise RouterUnreachableError(router_ip="デフォルトゲートウェイが見つかりません。")
...
def check_internet_connectivity(test_url: str = "https://www.google.com", timeout: float = 5.0):
    """インターネット接続を確認します。"""
    try:
        response = requests.get(test_url, timeout=timeout)
        if response.status_code != 200:
            raise InternetConnectionError(f"URL {test_url} へのアクセスに失敗しました。ステータスコード: {response.status_code}")
    except requests.RequestException as e:
        raise InternetConnectionError(f"インターネット接続に失敗しました。エラー: {e}") from e

def perform_connectivity_checks(device_ip: str, test_url: str = "https://www.google.com"):
    """全ての接続チェックを実行します。"""
