...
from NatureRemoMatterControl.connectivity_checker import (
    get_default_gateway,
    ping,
    check_router,
    check_device,
...
def test_check_router_success():
    with patch('NatureRemoMatterControl.connectivity_checker.ping', return_value=True):
        # Expect no exception
        check_router('192.168.1.1')

def test_check_router_failure():
    with patch('NatureRemoMatterControl.connectivity_checker.ping', return_value=False):
...
def test_perform_connectivity_checks_success():
    with patch('NatureRemoMatterControl.connectivity_checker.get_default_gateway', return_value='192.168.1.1'):
        with patch('NatureRemoMatterControl.connectivity_checker.check_router') as mock_check_router:
            with patch('NatureRemoMatterControl.connectivity_checker.check_device') as mock_check_device:
                with patch('NatureRemoMatterControl.connectivity_checker.check_internet_connectivity') as mock_check_internet:
...
def test_perform_connectivity_checks_failure():
    with patch('NatureRemoMatterControl.connectivity_checker.get_default_gateway', return_value='192.168.1.1'):
        with patch('NatureRemoMatterControl.connectivity_checker.check_router') as mock_check_router:
            with patch('NatureRemoMatterControl.connectivity_checker.check_device', side_effect=DeviceUnreachableError('192.168.1.2')):
                with pytest.raises(DeviceUnreachableError) as exc_info:
                    perform_connectivity_checks('192.168.1.2')
                assert "指定されたIPアドレス 192.168.1.2 に接続できません。" in str(exc_info.value)
